...
const Module = (function () {
	const modulePrivateData = new WeakMap();
	...
	class Module extends PubSub {
		...
		static getDependencies(fnStr) {
			...
			matched = matched.map((match) => {
				const splitted = match.split('._.');
				return splitted[1] || splitted[0];
			});

			// If any evaluated dependency is present at root level
			const evaluatedDependencyAtRoot = matched.find(match => match.startsWith('._['));
		...
		$proxyHandler() {
			const ctx                   = this;
			const _callObservingMethods = function () {
				setTimeout(() => {
		 ...
		 */
		dequeueEvents() {
			const moduleSubscriptions = this.getAllSubscriptions();
			eventQ.store.forEach((evt) => {
				const queuedEvent = moduleSubscriptions.filter((event) => {
